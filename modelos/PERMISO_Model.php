...
class Permiso_modelo
{
    ...
    public static function getPermisosPerfil($perfil)
    {
        $conexion = conectarBD();
    ...
    public static function getPermisosControlador($controlador){
        $conexion = conectarBD();
        if($controlador==""){
            $sql= "SELECT * from permisos";
        }else {
            $sql = "SELECT * from permisos WHERE controlador='" . $controlador . "';";
        }
        return $conexion->query($sql);
    }
}
